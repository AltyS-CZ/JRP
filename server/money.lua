...
function LoadPlayerInventory(identifier)
    exports.oxmysql:fetch('SELECT * FROM player_inventory WHERE identifier = ?', {identifier}, function(result)
        if result then
            -- Assuming you have a table to store player's inventory in memory
            player_inventories[identifier] = {}
            for i = 1, #result do
                player_inventories[identifier][result[i].item] = result[i].count
            end
        else
            player_inventories[identifier] = {}
        end
